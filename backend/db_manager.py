...
class DBManager:
    ...
    def _connect(self):
        # Establish connection with 30s busy timeout and enable WAL mode to avoid lock issue.
        conn = aiosqlite.connect(self.db_path, timeout=30.0)
        original_connect = conn._connect
        
        async def custom_connect():
            db_conn = await original_connect()
            try:
                await db_conn.execute("PRAGMA journal_mode=WAL;")
                await db_conn.execute("PRAGMA synchronous=NORMAL;")
            except Exception as e:
                print(f"[DB Warning] SQLite pragma failed: {e}")
            return db_conn
            
        conn._connect = custom_connect
        return conn
